...
% SLIDE 8: THE CLIFF
% ==========================================
\begin{frame}[t]{The Heat Transfer Cliff - The Nukiyama Curve}
    \begin{columns}[T,onlytextwidth]
        \column{0.50\textwidth}
        ...
        \column{0.45\textwidth}
	\centering
        % Using the extracted Nukiyama curve image
        \includegraphics[width=\linewidth, keepaspectratio]{media/extracted_nukiyama_curve.png}
%\begin{tikzpicture}[x=0.85cm,y=0.85cm]
%          % Axes
%          \draw[-{Stealth}, thick, color=gray] (0,0) -- (6.2,0) node[below, xshift=-2.5cm] {$\Delta T_{sat} = T_{wall}-T_{sat}$};
%          \draw[-{Stealth}, thick, color=gray] (0,0) -- (0,4.6) node[left] {$q''$};
%
%          % Origin Label (0) - Added here
%          \node[color=gray, font=\footnotesize, anchor=north east] at (0,0) {0};
%
%          % Curve
...
%
%          % Labels
%  \node[align=center, font=\tiny, color=HadronBlue] at (3.0,1.2) {Nucleate\\Boiling};
%          \node[align=center, font=\tiny, color=StatusRed] at (5.0,1.2) {Film\\Boiling};
%        \end{tikzpicture}
%
%        \begin{tikzpicture}[x=0.85cm,y=0.85cm]
% \draw[-{Stealth}, thick, color=gray] (0,0) -- (6.2,0) node[below, xshift=-2.0cm] {$\Delta T_{sat} = (T_{wall} - T_{sat})$};
%          \draw[-{Stealth}, thick, color=gray] (0,0) -- (0,4.6) node[left] {$q''$};
%          \draw[very thick, HadronBlue] (0.4,0.3) .. controls (1.6,0.9) and (2.4,1.5) .. (3.0,2.4)
%                       .. controls (3.4,3.0) and (3.7,3.7) .. (4.0,4.05);
%          \draw[very thick, StatusRed] (4.0,4.05) .. controls (4.3,3.2) and (4.7,2.4) .. (5.2,2.0)
...
\begin{frame}[t]{Scaling Considerations: From Core to Pin}
    \begin{columns}[T,onlytextwidth]
        ...
        Core and assembly-level thermal-hydraulics "smear" properties, masking the local extremes where DNB actually occurs.
...
\begin{frame}[t]{Final Takeaways}
    \begin{itemize}
        ...
        \item \textbf{Modeling:} Subchannel models are essential for PWR analysis to track  hydraulic behavior locally.
